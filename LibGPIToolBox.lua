...
local print = Addon.api.print
local LOCALIZED_CLASS_NAMES_MALE = Addon.api.LOCALIZED_CLASS_NAMES_MALE
...
-- Obszczymucha: LMFAO wtf is this :D
...
function Tool.Split( inputstr, sep )
  ...
  local t = {}
  for str in string.gmatch( inputstr, "([^" .. sep .. "]+)" ) do
    if table_contains( t, str ) == false then
      table.insert( t, str )
    end
  end
  return t
